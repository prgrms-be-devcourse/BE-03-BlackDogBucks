		...
		mockMvc.perform(post(BASE_REQUEST_URI)
			...
			.andDo(document("item-create",
				...
				requestFields(
					...
					fieldWithPath("defaultOption").type(JsonFieldType.OBJECT).description("디폴트 옵션"),
					fieldWithPath("defaultOption.espressoShotCount").type(JsonFieldType.NUMBER)
						.description("에스프레소 샷 카운트 수"),
					fieldWithPath("defaultOption.vanillaSyrupCount").type(JsonFieldType.NUMBER)
						.description("바닐라 시럽 샷 카운트 수"),
					fieldWithPath("defaultOption.classicSyrupCount").type(JsonFieldType.NUMBER)
						.description("클래식 시럽 샷 카운트 수"),
					fieldWithPath("defaultOption.hazelnutSyrupCount").type(JsonFieldType.NUMBER)
						.description("헤이즐넛 시럽 샷 카운트 수"),
					fieldWithPath("defaultOption.milkType").type(JsonFieldType.NULL).description("우유 타입"),
					fieldWithPath("defaultOption.espressoType").type(JsonFieldType.NULL).description("에스프레소 타입"),
					fieldWithPath("defaultOption.milkAmount").type(JsonFieldType.NULL).description("우유 양")
				)
			));
